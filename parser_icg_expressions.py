#parser and ICG code that works only for expressions such as 6 - 9 + ( 2 *3 ) ;
#will only work for expressions and not code snippets like function declarations
...
def parse(tokens):
    ...
    def program():
        if current_token['Type'] == 'Keyword' and current_token['Value'] == 'int':
            ...
            match('Symbol', ';')
            return Node('program', [expression_node])
    ...
    def statements():
        children = []
        while current_token and current_token['Value'] != 'return':
